...
- Python-basierter Abhängigkeits-Bootstrap für Linux x86_64 mit APT, DNF/YUM, Pacman und Zypper. `setup-linux.sh` bleibt als minimale Ausnahme für die Erkennung beziehungsweise Installation eines fehlenden System-Python erhalten und delegiert danach an `setup-linux.py`; `setup-ubuntu.sh` bleibt ein Kompatibilitätsalias.
- Generisches Diagnoseschema 3 und Linux-Setup-Schema 2 mit `coreRuntime` für Sprache, Mindestversion und Pfad der plattformspezifischen Kernruntime; die privaten Auftragsschemata bleiben unverändert.
- Generische Schema-1-Runtime-Fingerprints für beide Produktivkerne: PowerShell und Python schreiben zusätzlich zu den lesbaren Altfeldern Plattform, Sprache, Runtimeart, Mindestversion und ausführbaren Pfad. Dadurch können Gegenkernprüfungen fremde technische Nachweise zuverlässig entwerten.
...
- Windows-Dispatcher `Tools/bewerbung.ps1` einschließlich `universal-neu`, `universal-status` und `universal-finalisieren`; der Linux-Python-Dispatcher bildet denselben GNU-Langvertrag eigenständig ab.
...
- Dünne Linux-Launcher `Tools/bewerbung.sh` und `Tools/neue-bewerbung.sh` als Python-Aliase sowie `Tools/setup-ubuntu.sh` als Kompatibilitätsalias für den distributionsneutralen Bootstrap.
- Windows-PowerShell-Module und eigenständige Linux-Python-Module für portable Schema-5-Auftragspfade, symlinksichere Pfadvalidierung, OS- und Browsererkennung, begrenzte native Prozesse mit Timeout sowie standardbibliotheksbasierte PNG-Auswertung.
- Read-only-Preflight über den jeweiligen Plattform-Dispatcher und eine feste browserfreie CI mit PowerShell auf `windows-2025` sowie Python auf `ubuntu-24.04`; Browser-Smokes laufen getrennt.
...
- Das Subcommand `tests --suite ...` des jeweiligen Plattform-Dispatchers ist die gemeinsame Schnittstelle; `--mit-browser` bleibt als Browseralias lesbar. README, technischer Workflow und Agenten-Kompatibilitätsübersicht dokumentieren die schnelle/vollständige Teilung, die OpenCode-Canary und den Vorschau-Status von Linux bis zur Promotion.
...
- Die Agenten-, E-Mail- und Technikhinweise benennen jetzt explizit den privaten Kandidatenpfad, die vier inhaltlich erforderlichen Nachweise, den aus `firmaSlug` abgeleiteten Markdown-Dateinamen der E-Mail, die feste A4-CSS-Geometrie und das Subcommand `finalisieren` des Plattform-Dispatchers als einzigen Standardweg. Dadurch dürfen lokale Modelle weder Dummy-Dateien noch HTML-E-Mails oder lose Direkt-Exporte als zulässige Abkürzung interpretieren.
- Windows verwendet weiterhin PowerShell 7.6 als fachlichen Kern; Linux verwendet den eigenständigen Python-3.9+-Kern. Bash enthält keine Bewerbungsartefakt-, Hash- oder fachliche Workflowlogik; nur der minimale Python-Bootstrap darf seinen eigenen Setup-Plan ausgeben und die Runtime-Installation delegieren.
- Ein bereits vorhandenes PowerShell 7.6 kann unter Linux vorübergehend mit `pwsh -NoProfile -File Tools/bewerbung.ps1 ...` als ausdrücklich benannter Legacy-Fallback direkt aufgerufen werden. Die Linux-Aliase und die Linux-CI verwenden ausschließlich Python; das Linux-Setup installiert PowerShell nicht mehr.
- Der ungenutzte PowerShell-Archivdownload für Linux wurde entfernt. Für Linux sind ausschließlich System-Python sowie die deklarierten Distributionspakete ein Installationsziel; der vorhandene PowerShell-Weg bleibt rein manuell als Legacy-Fallback.
...
- Designvorlagen verwenden zusätzlich `Liberation Sans`; Windows verlangt PowerShell 7.6 Core und Linux System-Python 3.9+.
...
- Das Linux-Setup verwendet ausschließlich Distributionspakete für Python, Chromium, Liberation Sans und ShellCheck. PyPI, virtuelle Umgebungen, Snap, AUR und fremde Browserquellen bleiben ausgeschlossen; `--dry-run`, Bestätigung und idempotente Erkennung verhindern stille Systemänderungen.
- Pacman weist die vollständige Systemaktualisierung des ersten Installationsschritts mit `-Syu` im Plan aus. Blockierte Ubuntu- und RHEL-kompatible Browserwege enthalten eine strukturierte `manualInstruction` mit exaktem Diagnosebefehl; Liberation Sans wird auch ohne `fc-match` über eine tatsächliche reguläre Fontdatei validiert.
...
- Plattformübergreifende Zustandsaktualisierungen respektieren exklusive Dateisperren beim atomaren Ersetzen; der Bash-Dispatcher-Test erwartet die verbindliche absolute Pfadnormalisierung zum Linux-Python-Kern.
...
- Headless-Chrome-Export und Layoutcheck behalten auf normalen Windows- und Linux-Hosts die Chromium-Sandbox aktiv; die früheren pauschalen `--no-sandbox`-/`--disable-gpu-sandbox`-Flags wurden entfernt. Linux-Root-Läufe werden fail-closed abgelehnt. Nur die ephemeren Root-Container der Distributions-CI aktivieren über `APPLY_FOUNDRY_ALLOW_UNSANDBOXED_BROWSER=1` eine ausdrücklich begrenzte Testausnahme.
...
- Linux-CI führt Python-Unit-/Vertragstests und die Dispatcher-Suiten ohne `pwsh` aus; Windows behält die bestehenden PowerShell-Suiten. Die zeitgesteuerte Distributionsmatrix umfasst Ubuntu 24.04/26.04, Debian 13, Fedora, Rocky 9, Arch und openSUSE und lädt bereinigte Vollsuite- sowie verfügbare Browserberichte je Zielprofil hoch; Ubuntu-Snap sowie die in Rocky-Base-Repositories fehlenden Browser-/ShellCheck-Pakete werden als blockierte Voraussetzungen geprüft, ohne Snap oder EPEL zu registrieren. Der bisherige Stabilitätscollector bleibt bis zu seiner separaten Erweiterung ein Ubuntu-24.04-Entwurf und gilt nicht als Profil-Promotion.
- Prompt-Canary und vollständige Promptmatrix verwenden auf sauberen ephemeren Ubuntu-Runnern den nativen Python-Runner mit gepinnten CLI-Versionen, gezielter Credential-Weitergabe, Mutations- und Signalkontrolle. `bubblewrap` wird ausschließlich im Testworkflow installiert; Versionsprobe und Agentlauf sind in Mount-/PID-/User-Namespaces mit unsichtbarem Host-Home, read-only Systemruntimes und nur für Provider-APIs geteiltem Netz eingeschlossen. Fehlendes oder unbrauchbares `bwrap` endet fail-closed. Kataloggebundene Modellargumente sind Pflicht; maschinenlesbar ausgewiesene tatsächliche Modelle müssen exakt passen. Die Rollenvalidatoren verlangen neu erzeugte Matrix-/Evidenzartefakte mit Schema-, SHA-, Evidenz- und Strategieprüfung sowie im direkten Fall ausgewählte A4-/Platzhalterprüfungen. Nur der read-only Actions-Evidence-Sammler bleibt als Windows-PowerShell-Hilfsjob bestehen.
- Eine neue Cross-Core-CI erzeugt aus denselben öffentlichen synthetischen Quellen unter Windows/PowerShell und Linux/Python jeweils einen Schema-5-Auftrag, lädt den vollständigen synthetischen Zustand und echte Ursprungs-Runtime-Fingerprints hoch, setzt Status und Checkpoint auf dem Gegenkern fort, lehnt fremde technische Schema-1-Fingerprints mit den produktiven Validatoren ab und vergleicht die normalisierten Aufträge unter Ubuntu bytegenau. Der Nachweis deckt diese Portabilitätsverträge ab, ohne daraus bereits vollständige Rollenfixture- oder Browserparität abzuleiten.
- Die lokale native Python-Browser-Suite lief unter Ubuntu 26.04 mit Chrome 151 vollständig grün: 87 synthetische Fälle deckten zusätzlich zur A4-/PNG-/PDF-/ATS-Prüfung die hashgebundene Freigabe und die atomare Veröffentlichung einer normalen sowie einer universellen Bewerbung ab. Der noch ausstehende Rollout-Nachweis sind drei aufeinanderfolgende CI-Läufe je Linux-Zielprofil.
- Der neue Linux-Setup-Vertrag bestand lokal mit 28 Python-Unit-Tests sowie den Bash-Suiten für den distributionsneutralen Bootstrap und den Ubuntu-Kompatibilitätsalias; dabei wurden keine Pakete installiert.
