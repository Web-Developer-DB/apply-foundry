#!/usr/bin/env python3
"""Installationsplan und Bootstrap für deklarierte Desktop-Abhängigkeiten.

Das Modul verwendet ausschließlich die Python-Standardbibliothek. Paketnamen
und Quellen stammen aus dependencies.json; Paketmanager werden immer mit
...
CATALOG_PATH = Path(__file__).with_name("dependencies.json")
...
def minimum_version_tuple(value: str) -> Tuple[int, int]:
    ...
    if match is None:
        raise SetupError(
            "Das Abhängigkeitsmanifest enthält eine ungültige Python-Mindestversion.",
...
def load_catalog(path: Path = CATALOG_PATH) -> Dict[str, Any]:
    try:
        ...
        raise SetupError(
            "Abhängigkeitsmanifest konnte nicht gelesen werden: {0}".format(exc),
            EXIT_USAGE_OR_PLATFORM,
        ) from exc

    if not isinstance(raw, dict) or type(raw.get("schemaVersion")) is not int:
        raise SetupError("Abhängigkeitsmanifest besitzt kein gültiges Schema.", EXIT_USAGE_OR_PLATFORM)
    if raw.get("schemaVersion") not in (1, 2) or raw.get("kind") not in ("apply-foundry-linux-dependencies", "apply-foundry-dependencies"):
        raise SetupError("Abhängigkeitsmanifest verwendet einen unbekannten Vertrag.", EXIT_USAGE_OR_PLATFORM)
    ...
    if raw.get("schemaVersion") == 2:
        if not isinstance(platforms, dict) or set(platforms) != {"windows", "macos"}:
            raise SetupError("Generisches Abhängigkeitsmanifest enthält keine Windows-/macOS-Definition.", EXIT_USAGE_OR_PLATFORM)
        for platform_name, expected_manager in (("windows", "winget"), ("macos", "brew")):
            definition = platforms[platform_name]
            if not isinstance(definition, dict) or definition.get("packageManager") != expected_manager:
                raise SetupError("Generisches Abhängigkeitsmanifest enthält einen ungültigen Paketweg.", EXIT_USAGE_OR_PLATFORM)
            components = definition.get("components")
            if not isinstance(components, dict) or set(components) != set(COMPONENT_NAMES):
                raise SetupError("Generisches Abhängigkeitsmanifest ist unvollständig.", EXIT_USAGE_OR_PLATFORM)
    return raw
...
def build_context(
    ...
    for manager_name in catalog["managerPrecedence"]:
        ...
        if executable_path:
            ...
            if trusted_path:
                return PlatformContext(distro_id, distro_version, architecture, manager_name, trusted_path, "linux")
    raise SetupError(
...
def build_platform_context(catalog: Mapping[str, Any]) -> PlatformContext:
    """Return the declared package route without guessing unknown platforms."""
    ...
    if sys.platform == "win32":
        ...
        if not executable:
            raise SetupError("winget fehlt. Installiere Python 3.13, Google Chrome und optional ShellCheck manuell oder über App Installer; danach erneut starten.", EXIT_USAGE_OR_PLATFORM)
        return PlatformContext("windows", platform.version(), architecture, "winget", executable, "windows")
    if sys.platform == "darwin":
        executable = shutil.which("brew")
        if not executable:
            raise SetupError("Homebrew fehlt. Installiere Python 3.13, Google Chrome, ShellCheck und bei Bedarf Liberation Sans manuell; danach erneut starten.", EXIT_USAGE_OR_PLATFORM)
        return PlatformContext("macos", platform.mac_ver()[0] or platform.version(), architecture, "brew", executable, "macos")
    raise SetupError("Unterstützt werden Windows, Linux und macOS.", EXIT_USAGE_OR_PLATFORM)


def platform_components(context: PlatformContext, catalog: Mapping[str, Any]) -> Mapping[str, Mapping[str, Any]]:
    if context.platform_id == "linux":
        return catalog["packageManagers"][context.manager]["components"]
    return catalog["platforms"][context.platform_id]["components"]
...
def python_candidates(which: Callable[[str], Optional[str]] = shutil.which) -> Iterable[str]:
    ...
    for candidate in candidates:
        try:
            ...
            continue
        # The interpreter currently executing this verified local bootstrap is
        # a valid runtime candidate on Windows/macOS, where Unix ownership
        # checks are not meaningful. Other discovered executables retain the
        # strict system-owner check on Linux.
        trusted = normalized if (sys.prefix == getattr(sys, "base_prefix", sys.prefix) and os.path.normcase(normalized) == os.path.normcase(str(Path(sys.executable).resolve())) ) else resolve_trusted_executable(normalized)
...
def collect_detections(catalog: Mapping[str, Any]) -> Mapping[str, Detection]:
    ...
    if sys.platform == "win32":
        fonts = Path(os.environ.get("WINDIR", r"C:\\Windows")) / "Fonts" / "arial.ttf"
        browser = next((Path(value) for value in (
        ...
        return {"coreRuntime": detect_python(minimum), "browser": Detection(browser is not None, path=str(browser) if browser else None, provider="Google Chrome"), "fonts": Detection(fonts.is_file(), path=str(fonts) if fonts.is_file() else None), "shellcheck": detect_shellcheck(trust_executable=lambda value: value)}
    if sys.platform == "darwin":
        chrome = Path("/Applications/Google Chrome.app/Contents/MacOS/Google Chrome")
        fonts = (Path("/System/Library/Fonts/Supplemental/Arial.ttf"), Path("/Library/Fonts/Arial.ttf"), Path("/Library/Fonts/LiberationSans-Regular.ttf"))
        font = next((item for item in fonts if item.is_file()), None)
        return {"coreRuntime": detect_python(minimum), "browser": Detection(chrome.is_file(), path=str(chrome) if chrome.is_file() else None, provider="Google Chrome"), "fonts": Detection(font is not None, path=str(font) if font else None), "shellcheck": detect_shellcheck(trust_executable=lambda value: value)}
    return {
...
def distribution_block_reason(
    ...
    if detection.present:
        return None
    if context.platform_id == "windows" and component_name == "fonts":
        return "Arial ist eine geprüfte Windows-Systemvoraussetzung und wird nicht automatisch ersetzt."
...
def build_report(
    ...
    manager_components = platform_components(context, catalog)
    ...
    for name in COMPONENT_NAMES:
        ...
        component: Dict[str, Any] = {
            ...
            "permission": "administrator" if context.platform_id == "windows" else "root-or-sudo" if context.platform_id == "linux" else "brew-or-administrator",
            "installable": not blocked,
            "blocked": blocked,
            "plannedAction": planned_action,
        }
        if name == "coreRuntime":
            component["platform"] = context.platform_id
            component["language"] = "python"
            component["minimumVersion"] = "3.11"
            component["setupCommand"] = "python3 Tools/setup.py --runtime --dry-run --format json"
...
class CommandRunner:
    def run(self, args: Sequence[str], env: Optional[Mapping[str, str]] = None) -> None:
        try:
...
class PrivilegeExecutor:
    def __init__(
        ...
        self.runner = runner
        self.effective_uid = (os.geteuid() if hasattr(os, "geteuid") else 0) if effective_uid is None else effective_uid
...
class PackageInstaller:
    ...
    def install(self, packages: Sequence[str]) -> None:
        ...
        if manager == "apt":
            ...
            # winget is the sole Windows package path.  It may elevate itself;
            # no shell and no third-party source is involved.
            self.privilege.runner.run((executable, "install", "--id", packages[0], "--exact", "--source", "winget", "--accept-source-agreements", "--accept-package-agreements"))
        elif manager == "brew":
            # Avoid a surprise global package update in a project bootstrap.
            command = [executable, "install"]
            if packages[0] in ("google-chrome", "font-liberation"):
                command.append("--cask")
            command.extend(packages)
            environment = dict(os.environ)
            environment["HOMEBREW_NO_AUTO_UPDATE"] = "1"
            self.privilege.runner.run(command, environment)
        else:
...
def parse_args(argv: Sequence[str]) -> Tuple[argparse.ArgumentParser, Options]:
    parser = argparse.ArgumentParser(
        prog="setup.py",
        description="Deklarierte Apply-Foundry-Abhängigkeiten plattformneutral planen oder installieren.",
    )
    parser.add_argument("--runtime", action="store_true", help="System-Python 3.11 oder neuer")
    parser.add_argument("--browser", choices=("chromium",), help="Chromium/Chrome aus dem deklarierten Paketweg")
    parser.add_argument("--fonts", action="store_true", help="plattformgeeignete Schriftart")
...
def run_setup(options: Options, context: PlatformContext, catalog: Mapping[str, Any]) -> int:
    ...
    emit(final, options.output_format, "Setup erfolgreich abgeschlossen:")
    if options.output_format == "text":
        if applied_changes:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    if sys.version_info < (3, 11):
        print(
            "FEHLER: System-Python 3.11 oder neuer ist erforderlich. "
            "Starte Tools/setup.sh --runtime --dry-run.",
            file=sys.stderr,
        )
        return EXIT_USAGE_OR_PLATFORM
    ...
    try:
        ...
        context = build_platform_context(catalog)
