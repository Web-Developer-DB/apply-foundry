...
Prüfe Fähigkeiten, nicht Anbieternamen. Kläre bedarfsgerecht, ob die Agentenumgebung Dateien lesen und schreiben, Terminalbefehle ausführen, unter Windows PowerShell 7.6 oder unter Linux System-Python 3.9+ verwenden, den jeweiligen Plattform-Dispatcher starten, Chrome, Edge oder Chromium ausführen, PNG-Dateien tatsächlich visuell auswerten und maschinenlesbare Nutzungsdaten liefern kann. Beachte Sandbox- und Berechtigungsgrenzen.
...
Vor einem Start-, Reparatur- oder Testauftrag prüft der Agent die deklarierten Projektabhängigkeiten read-only. Unter Linux sind dies Python 3.9+, Chromium, Liberation Sans und optional ShellCheck; unter Windows PowerShell 7.6, ein Chromium-Browser, Arial und optional ShellCheck. Der kanonische Linux-Dry-run lautet `python3 Tools/setup-linux.py --all --dry-run --format json`; fehlt Python selbst, verwendet der Agent dafür einmalig den minimalen Bootstrap `Tools/setup-linux.sh`. Unter Windows gilt `Tools/setup-windows.ps1 -All -DryRun -Format json`. Bei einem klaren Projektauftrag darf er den angezeigten Plan nach bestätigter Berechtigung mit `--yes` beziehungsweise `-Yes` ausführen; ohne Berechtigung nennt er den exakten Befehl und wartet. Installiert werden ausschließlich diese deklarierten Projektkomponenten aus den dokumentierten Distributions- beziehungsweise winget-Quellen. PyPI, virtuelle Umgebungen, Snap, AUR, Editor-Erweiterungen, Agenten-Plugins und beliebige weitere Pakete werden weder gesucht noch automatisch installiert.

Ein Pacman-Plan weist ausdrücklich aus, dass der erste Installationslauf mit `-Syu` das vollständige System aktualisiert; auch diese breitere Änderung darf erst nach dem angezeigten Plan und der bestätigten Berechtigung ausgeführt werden. Blockierte Ubuntu- oder RHEL-kompatible Browserwege werden nicht umgangen: Der Agent übernimmt `manualInstruction` und den exakten `verificationCommand` aus dem Setup-Plan. Eine erkannte Liberation Sans gilt nur mit validierter tatsächlicher Fontdatei als vorhanden, auch wenn `fc-match` fehlt.

Ist unter Linux bereits PowerShell 7.6 vorhanden, darf `pwsh -NoProfile -File Tools/bewerbung.ps1 ...` ausschließlich als ausdrücklich benannter Legacy-Fallback zur Migrationsdiagnose verwendet werden. Die Linux-Aliase und der normale Linux-Workflow bleiben Python-basiert; der Linux-Bootstrap installiert PowerShell nicht.
...
- Starte für neue oder fortgesetzte Bewerbungen nicht direkt ein Einzelwerkzeug wie `Exportiere-PDF.ps1`, sondern den vollständigen Plattform-Dispatcher: unter Windows `Tools/bewerbung.ps1 finalisieren`, unter Linux `python3 Tools/bewerbung.py finalisieren`. Einzelwerkzeuge dienen nur der gezielten Diagnose und lockern keinen dieser Verträge.
