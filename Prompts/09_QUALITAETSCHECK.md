...
Vor dem Subcommand `finalisieren` des einheitlichen Python-Dispatchers (`python3 Tools/bewerbung.py`) werden technische Ergebnisse als `ausstehend` behandelt. Der Agent darf keine Browser-, Screenshot-, PDF-, ATS- oder Sichtprüfung als bestanden beschreiben, die nicht tatsächlich im aktuellen Lauf stattgefunden hat.
